...
void ReadCPUInformation(Tuplestorestate *tupstore, TupleDesc tupdesc)
{
	...
	if (sysctlbyname("hw.model", &cpu_type, &int_size, 0, 0) == -1)
	...
	if (sysctlbyname("hw.ncpu", &no_of_cores, &int_size, 0, 0) == -1)
		nulls[Anum_no_of_cores] = true;

	if (sysctlbyname("hw.clockrate", &cpu_frequency, &uint64_size, 0, 0) == -1)
